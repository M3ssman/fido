...
"""
  ...
  mock_request.Request.side_effect = lambda url, data=None: FakeRequest(url, data=data)
...
def test_get_droid_signatures_handles_http_error(mock_request, capsys):
  ...
  mock_request.urlopen.side_effect = urllib.error.HTTPError(url="http://example.test", code=500, msg="boom", hdrs=None, fp=None)
